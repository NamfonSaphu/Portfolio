...
export default function SkillPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-10 sm:px-6 lg:px-8 bg-gradient-to-b from-slate-50 to-slate-100 rounded-lg">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {category.skills.map((skill, index) => (
                    <motion.div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-300 ${hoveredSkill === `${category.name}-${skill.name}`
                    ...
                    >
                      <div
                        className={`p-2 rounded-md ${hoveredSkill === `${category.name}-${skill.name}`
                      ...
                      <span
                        className={`font-medium ${hoveredSkill === `${category.name}-${skill.name}` ? category.textHoverColor : "text-slate-700"
                          } transition-colors duration-300`}
                      >
                        {skill.name}
                      </span>
